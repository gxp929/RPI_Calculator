...
# Language dictionary
translations = {
    "en": {...},  # OMITTED HERE FOR BREVITY — you have full version
    "zh": {...},
    "ms": {...}
}
...
with col1:
    ...
    with st.expander(t["property_section"], expanded=True):
        ...
        monthly_service_fee_per_sqf = st.number_input(t["monthly_service_fee"], value=0.66, min_value=0.0, format="%.4f")
...
if st.button(t["calculate"], type="primary"):
    with st.spinner(t["processing"]):
        time.sleep(0.5)  # Shorter sleep for nicer UX
    ...
    st.subheader(f"{t['results']} {t['in_foreign_currency']} {foreign_currency}")
